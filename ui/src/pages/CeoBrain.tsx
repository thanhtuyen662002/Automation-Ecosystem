...
// Data source: /api/v1/strategy/* — NO mock data.
...
import {
  useStrategy, useNiches, useRecommendations, useStrategyLog, useUpdateStrategy,
} from '@/lib/hooks';
...
export function CeoBrain() {
  ...
  // Sync edit form whenever strategy data arrives
  useEffect(() => {
    if (state) {
      setEditValues({ ...state });
      setProposedMode(state.growth_mode ?? 'balanced');
    }
  }, [state]);

  function applyState() {
    updateStrategy.mutate(
      { ...editValues, growth_mode: proposedMode },
      { onSuccess: () => setShowStateEditor(false) },
    );
  }

  if (stateLoading || !state) return <LoadingCard />;
  ...
  const actionableRecs = (recommendations as any[]).map((r: any, i: number) => ({
    ...r,
    actionLabel: [t('ceo.act_spawn'), t('ceo.act_budget'), t('ceo.act_niches')][i] ?? null,
  }));
  ...
  return (
    <div>
        ...
        action={
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            <button className="btn btn-primary btn-sm" onClick={() => setShowStateEditor(true)}>
              <Brain size={13} /> {t('ceo.act_adjust')}
            </button>
          </div>
        }
      />
        ...
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem' }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontWeight: 700, color: 'var(--info)', fontVariantNumeric: 'tabular-nums' }}>{state.consecutive_low_cycles ?? 0}</div>
      ...
      >
        {showLog ? <ChevronUp size={12} /> : <ChevronDown size={12} />} {t('ceo.log_title')} ({strategyLog.length} {t('ceo.log_entries')})
      </button>
      {showLog && (
        <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div>
            <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.375rem' }}>
              {t('ceo.lbl_target_views')} {((editValues.target_daily_views ?? 0) / 1000).toFixed(0)}K
            </label>
            <input type="range" min={10000} max={500000} step={5000} value={editValues.target_daily_views ?? 50000}
              onChange={e => setEditValues(v => ({ ...v, target_daily_views: +e.target.value }))}
              style={{ width: '100%', accentColor: 'var(--primary)' }} />
          </div>
          <div>
            <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.375rem' }}>
              {t('ceo.lbl_target_rev')} {fmtCurrency(editValues.target_daily_revenue ?? 0)}
            </label>
            <input type="range" min={5} max={500} step={5} value={editValues.target_daily_revenue ?? 50}
