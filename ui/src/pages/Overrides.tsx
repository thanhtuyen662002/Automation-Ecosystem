...
export function Overrides() {
  ...
  function handleRemove(target_id: string) {
    removeOverride.mutate(target_id, { onSuccess: () => setConfirmRemove(null) });
  }
  function handleAddPreset() {
    ...
    addOverride.mutate(
      { target_id: targetId, target_type: selectedPreset.target_type, override: selectedPreset.override, reason: t(selectedPreset.descKey), ttl_hours: selectedPreset.ttl_hours },
      { onSuccess: () => { setSelectedPreset(null); setPresetTarget(''); setShowAdd(false); } },
    );
  }
  function handleAddCustom() {
    if (!form.target_id) return;
    addOverride.mutate({ ...form }, { onSuccess: () => { setForm({ target_id: '', target_type: 'account', override: 'freeze', reason: '', ttl_hours: 24 }); setShowCustom(false); } });
  }

  return (
    <div>
      <PageHeader title={t('override.title')} subtitle={t('override.sub')}
        action={
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button className="btn btn-primary btn-sm" onClick={() => setShowAdd(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <GlassIcon name="fire" size={14} style={{ filter: 'brightness(0) invert(1)' }} />
              {t('override.act_quick')}
            </button>
      ...
      {isLoading && (
        <div style={{ textAlign: 'center', padding: '2rem' }}>
          <GlassIcon name="arrows-square-up-down" size={36} style={{ opacity: 0.3, marginBottom: '0.5rem' }} />
          <div style={{ color: 'var(--text-muted)' }}>Loading overrides…</div>
        </div>
      )}
      {error && (
        <div style={{ padding: '1rem', color: 'var(--danger)', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          <GlassIcon name="warning" size={15} style={{ filter: 'brightness(0) saturate(100%) invert(26%) sepia(90%) saturate(3000%) hue-rotate(345deg)' }} />
          {(error as Error).message}
        </div>
      )}

      {!isLoading && active.length === 0
        ? <EmptyState icon="check-circle" message={t('override.no_data')} />
        : (
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.375rem', flexWrap: 'wrap' }}>
                        ...
                        <code style={{ fontSize: '0.8125rem', color: 'var(--primary)', background: 'var(--primary-muted)', padding: '0.125rem 0.5rem', borderRadius: 4 }}>{o.target_id}</code>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '1rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                        ...
                        <span style={{ color: ttlLeft < 2 ? 'var(--danger)' : 'var(--text-muted)' }}>{ttlLeft.toFixed(1)}{t('override.lbl_remain')}</span>
                      </div>
                    </div>
      ...
      <SlideOver open={showAdd} onClose={() => { setShowAdd(false); setSelectedPreset(null); setPresetTarget(''); }} title={t('override.act_quick')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
          <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', marginBottom: '0.25rem' }}>{t('override.desc_quick')}</div>
          {PRESETS.map(p => (
            <div key={p.labelKey} className="card-elevated"
              style={{ padding: '0.875rem', cursor: 'pointer', border: `1px solid ${selectedPreset?.labelKey === p.labelKey ? overrideColor[p.override] : 'var(--border)'}`, borderRadius: 'var(--radius)', background: selectedPreset?.labelKey === p.labelKey ? 'var(--surface-2)' : undefined }}
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1rem' }}>
              {selectedPreset.needsTarget && (
                <div style={{ marginBottom: '0.75rem' }}>
                  ...
                  <input className="input" placeholder={t('override.ph_target_id').replace('{target}', selectedPreset.target_type)} value={presetTarget} onChange={e => setPresetTarget(e.target.value)} />
                </div>
              )}
              ...
              {addOverride.isError && <div style={{ color: 'var(--danger)', fontSize: '0.8rem', marginBottom: '0.5rem' }}>{(addOverride.error as Error)?.message}</div>}
      ...
      <SlideOver open={showCustom} onClose={() => setShowCustom(false)} title={t('override.act_custom')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
              {OVERRIDE_TYPES.map(type => (
                <button key={type} onClick={() => setForm(f => ({ ...f, override: type }))} className={`btn btn-sm ${form.override === type ? 'btn-primary' : 'btn-secondary'}`}>{type}</button>
              ))}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <input type="range" min={1} max={168} step={1} value={form.ttl_hours} onChange={e => setForm(f => ({ ...f, ttl_hours: +e.target.value }))} style={{ width: '100%', accentColor: 'var(--primary)' }} />
          </div>
          {addOverride.isError && <div style={{ color: 'var(--danger)', fontSize: '0.8rem' }}>{(addOverride.error as Error)?.message}</div>}
      ...
      <ConfirmDialog
        open={!!confirmRemove} onClose={() => setConfirmRemove(null)}
