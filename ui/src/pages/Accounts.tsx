...
import { useAccounts, useCreateAccount, useDeleteAccount, useMarkSoftBan, useClearSoftBan } from '@/lib/hooks';
...
export function Accounts() {
  ...
  function handleAddAccount() {
    ...
    createAccount.mutate(
      ...
      { onSuccess: () => { setNewHandle(''); setNewProxy(''); setShowAdd(false); } },
    );
  }
  function handleDelete(id: string) {
    deleteAccount.mutate(id, { onSuccess: () => { setSelected(null); setConfirmDelete(null); } });
  }
  function handleMarkSoftBan(id: string) { markSoftBan.mutate(id, { onSuccess: () => setSelected(null) }); }
  function handleClearSoftBan(id: string) { clearSoftBan.mutate(id, { onSuccess: () => setSelected(null) }); }

  return (
    <div>
      <PageHeader title={t('acc.title')} subtitle={t('acc.sub')}
        action={
          <button className="btn btn-primary btn-sm" onClick={() => setShowAdd(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
      ...
      {!isLoading && !error && (
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          {accounts.length === 0
            ? <EmptyState icon="user" message={t('acc.no_data')} />
            : (
                        ...
                        <tr key={a.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(a)}>
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                              <GlassIcon name="user" size={16} style={{ opacity: 0.5, flexShrink: 0 }} />
                              <div>
                            ...
                            <div style={{ display: 'flex', gap: '0.375rem' }}>
                              {softBan
                                ? <button className="btn btn-ghost btn-icon btn-sm" title={t('acc.clear_ban')} onClick={() => handleClearSoftBan(a.id)}>
                                    <GlassIcon name="shield" size={14} style={{ opacity: 0.7 }} />
                                  </button>
        ...
        {selected && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
              <GlassIcon name="user" size={32} />
              <div style={{ fontWeight: 700, fontSize: '1rem' }}>{selected.account_handle}</div>
            </div>
      ...
      <SlideOver open={showAdd} onClose={() => setShowAdd(false)} title={t('acc.add')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {createAccount.isError && (
            <div style={{ color: 'var(--danger)', fontSize: '0.8rem' }}>{(createAccount.error as Error)?.message}</div>
          )}
          <button className="btn btn-primary" onClick={handleAddAccount} disabled={!newHandle || createAccount.isPending}>
