...
import { TrendingUp, DollarSign, Eye, Brain, CheckCircle, RefreshCw, AlertCircle } from 'lucide-react';
import {
...
export function ExecutiveDashboard() {
      ...
      <PageHeader
        ...
        action={
          <button className="btn btn-secondary btn-sm" onClick={refetchAll}>
            <RefreshCw size={13} /> {t('exec.refresh')}
          </button>
        }
      />
      <div style={{ textAlign: 'center', padding: '4rem 1rem', color: 'var(--danger)' }}>
        <AlertCircle size={40} style={{ marginBottom: '1rem', opacity: 0.7 }} />
        <div style={{ fontSize: '0.9375rem', fontWeight: 600, marginBottom: '0.5rem' }}>{t('exec.error_load')}</div>
  ...
  return (
    <div>
      ...
      {/* ── KPI Row — Glassmorp GlassKpiCard ────────────────────────────────── */}
      <div className="grid-kpi" style={{ marginBottom: '1.25rem' }}>
        <GlassKpiCard
      ...
      {/* ── Chart Row — with Glassmorp blob effect ───────────────────────────── */}
      <div className="grid-2" style={{ marginBottom: '1.25rem' }}>
        <ChartCard
          title={t('exec.chart_views')}
          blobLeft="rgba(139,92,246,0.38)"
          blobRight="rgba(20,184,166,0.28)"
          minHeight={220}
        >
          {viewsTrend.length === 0 ? (
            <EmptyState icon="📈" message={t('exec.no_trend')} />
          ) : (
            <>
              {(() => {
                ...
                return (
                  <div style={{ fontSize: '0.75rem', marginBottom: '0.5rem', color: onTrack ? 'var(--success)' : 'var(--warning)' }}>
                    {onTrack ? '✓' : '⚠'} {t('exec.projected')}: <strong>{(eodViews / 1000).toFixed(0)}K {t('exec.views')}</strong>
        ...
        <ChartCard
          title={t('exec.niche_alloc')}
          blobLeft="rgba(236,72,153,0.28)"
          blobRight="rgba(245,158,11,0.25)"
          minHeight={220}
        >
          {nicheList.length === 0 ? (
            <EmptyState icon="🥧" message={t('exec.no_niche')} />
          ) : (
      ...
      <div className="grid-2" style={{ marginBottom: '1.25rem' }}>
        <ChartCard title={t('exec.niche_win')} blobLeft="rgba(139,92,246,0.30)" minHeight={180}>
          {nicheList.length === 0 ? (
            <EmptyState icon="📊" message={t('exec.no_data')} />
          ) : (
        ...
        <div className="card">
          ...
          {funnelData.length === 0 ? (
            <EmptyState icon="🔽" message={t('exec.no_funnel')} />
          ) : (
      ...
      <div className="grid-2">
        <div className="card">
          ...
          {topContent.length === 0 ? (
            <EmptyState icon="🏆" message={t('exec.no_metrics')} />
          ) : (
              ...
              {topContent.map((v: any) => (
                <div key={v.video_id} style={{ padding: '0.75rem', background: 'var(--bg-soft)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>👁 {fmt(v.views ?? 0)}</span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>❤ {fmt(v.likes ?? 0)}</span>
        ...
        <div className="card">
          ...
          {recs.length === 0 ? (
            <EmptyState icon="💡" message={t('exec.no_recs')} />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
